...
public class CaseDto extends BaseDto {
	...
	private List<VisitImgEntity> list;

	private Map<String, List<VisitImgEntity>> map;
	...
	public JSONArray convertJsonList(List<VisitImgEntity> list)
		...
		if (null != list && list.size() > 0) {
			for (int i = 0; i < list.size(); i++) {
				...
				jo.put("remarks", null != list.get(i).getRemarks() ? list
						.get(i).getRemarks() : "");
	...
	public List<VisitImgEntity> getList() {
		return list;
	}

	public void setList(List<VisitImgEntity> list) {
		this.list = list;
	}

	public Map<String, List<VisitImgEntity>> getMap() {
		return map;
	}

	public void setMap(Map<String, List<VisitImgEntity>> map) {
		this.map = map;
	}

	public List<VisitImgEntity> getValue(String key) {
		List<VisitImgEntity> list = map.get(key);
		return list;
	}
