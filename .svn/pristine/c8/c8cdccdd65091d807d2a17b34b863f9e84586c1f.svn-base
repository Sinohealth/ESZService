...
public class SickDailyResultDto extends BaseDto implements Serializable {
	...
	public SickDailyResultDto() {
		super();
		dailyNotes = new ArrayList<>();
		unwellItems = new ArrayList<>();
	}
	...
	public JsonNode getResults() {
		...
		ArrayNode takeMedsNode = rootNode.putArray("takeMeds");
		...
		// 服药
		if (null != takeMeds) {
			for (TakeMedRecordEntity e : takeMeds) {
	...
	public static class DailyNoteElem implements Serializable {
		...
		@JsonIgnoreProperties("diseaseId")
	...
	public static class UnwellElem implements Serializable {
		...
		public JsonNode getItems() {
			...
			for (VisitItemEntity e : items) {
				...
				node.put("options",
						e.getOptions() == null ? "" : e.getOptions());
			}
